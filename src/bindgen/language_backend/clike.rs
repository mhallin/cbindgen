...
impl LanguageBackend for CLikeLanguageBackend<'_> {
    ...
    fn write_enum<W: Write>(&mut self, out: &mut SourceWriter<W>, e: &Enum) {
        ...
        if has_data {
            ...
            if !inline_tag_field && self.config.language != Language::Csharp {
                out.write("union");
                out.open_brace();
            }
    ...
    fn write_union<W: Write>(&mut self, out: &mut SourceWriter<W>, u: &Union) {
        ...
        let list_type = if self.config.language == Language::Csharp {
            ListType::Wrap("[FieldOffset(0)]", ";")
        } else {
            ListType::Cap(";")
        };
        out.write_vertical_source_list(self, &u.fields, list_type, Self::write_field);
    ...
    fn write_opaque_item<W: Write>(&mut self, out: &mut SourceWriter<W>, o: &OpaqueItem) {
        ...
        if self.config.language == Language::Csharp {
            write!(out, "public struct {}", o.export_name());
            out.open_brace();
